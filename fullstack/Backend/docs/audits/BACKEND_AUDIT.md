...
## 24. Agency Feature & Checkout Bugfix (Phase 1–10, branch `main`)

### 24.1 Booking→Order/Payment reporting rewrite (Phase 1)
- `app/Queries/ReportQuery.php` fully rewritten: no more `Booking` models. Revenue/bookings KPIs join `orders` + `payments` (filter `payments.status = 'paid'`).
- `kpis()` returns associative shape `{revenue (float USD), currency, bookings, users, growth_percent}` — matches `resources/views/reports/booking-report.blade.php` + `GenerateReportService`.
- Method names expected by `GenerateReportService`: `newUsers`, `newUsersTrend`, `returningUsersTrend`, `topDestinations($from,$to)`, `topRevenueDestinations($from,$to)`; `weeklyRevenue` key `week_start`; `activeUsersTrend` key `active_users`.
- Feature test: `tests/Feature/System/ReportTest.php::test_report_reflects_real_order_and_payment_data` (real Order + OrderItem + Payment, asserts revenue == 500).

### 24.2 Domain cleanup (Phase 1)
- Migration `database/migrations/2026_08_09_000000_drop_experience_and_company_domain_tables.php` drops: `experience_providers`, `experiences`, `entity_views`, `companies`, `commissions`, `booking_items`, `bookings`, `transactions`.
- **SQLite workaround (important):** when `DB_CONNECTION=sqlite` the migration DROPS and RECREATES `payments` WITHOUT `booking_id` (SQLite cannot drop `bookings` while `payments.booking_id` FK exists, even with constraints disabled). Non-sqlite: `dropForeign(['booking_id'])` + `dropColumn`. `down()` intentionally empty (one-way).
- Deleted models: `Commerce\Booking`, `BookingItem`, `Commission`, `Transaction`. `Payment` no longer has `booking_id`/`booking()`.
- Deleted stale seeders (crashed on deleted models): `ExperienceProviderSeeder`, `ExperienceSeeder`, `CompanySeeder`, `BookingItemSeeder`, `EntityViewSeeder`, `BookingSeeder`, `TransactionSeeder`, `CommissionSeeder` (existing stale calls removed from `DatabaseSeeder`). `PaymentSeeder` rewritten to seed from `Order`. `AddressSeeder` second block (Experience addresses) removed.

### 24.3 Trip package fulfillment (Phase 2)
- `app/Listeners/FulfillOrderListener.php`: `elseif` branch for `trip_package` → `fulfillTripPackage($userId, $tripId)`.
- Idempotent: skips if trip already `BOOKED`/`COMPLETED`; transitions `TripStatus::BOOKED`; sends `TripBookedNotification` (new, `app/Notifications/TripBookedNotification.php`).
- Test: `tests/Feature/Commerce/PaymentFlowTest.php::test_payment_fulfillment_books_trip_package` (order must be `status = PAID` for the listener to proceed).

### 24.4 Restaurant pricing in trip packages (Phase 3)
- Migration `2026_08_09_221249_add_price_cents_to_restaurants_table.php`: `price_cents` on `restaurants`.
- `RestaurantSeeder`: `$`→15000–30000, `$$`→30000–60000, `$$$`→60000–120000.
- `app/Strategies/Checkout/TripPackageStrategy.php::calculatePrice()` sums restaurant `price_cents` (already cents — no ×100). Attractions remain unpriced (documented).

### 24.5 Agency domain (Phases 4–7)
- Migration `2026_08_09_221445_create_agency_assignments_table.php`: `agency_assignments` (+ `trips.agency_assignment_id` FK).
- Enum `AgencyAssignmentStatus`: `requested → admin_approved → agency_approved | agency_declined → completed | cancelled`.
- Model `Commerce\AgencyAssignment`: belongsTo customer/agency/admin (User), hasMany trips.
- Role `agency` + permissions `catalog.{hotels,restaurants,attractions,flights,destinations}.view` (Spatie, guard `api`).
- Events: `AgencyAssignmentAdminApproved`, `AgencyAssignmentApproved`, `AgencyAssignmentDeclined`.
- Policy `AgencyAssignmentPolicy`: `view` (customer/agency/admin/super_admin), `approve` (admin+requested), `respondAsAgency` (assigned agency+admin_approved), `cancel` (customer).
- Service `AgencyAssignmentService` + `AgencyAssignmentRepository` (bound in `AppServiceProvider`). Trip materialization sets sensible defaults (`travel_style=custom`, `no_of_travelers=1`, budget from level, dates now+7/+14); item `type` accepts full class OR morph-map short key (`Relation::getMorphedModel`).
- **Fix outside plan:** `app/Exceptions/ApiExceptionHandler::handleAuthorizationException()` widened to `AuthorizationException|AccessDeniedHttpException` — the 403 path previously threw TypeError → 500 (pre-existing bug surfaced by policy denies).
- `Trip` fillable gained `agency_assignment_id`.
- API (all `auth:api`, prefix `v1`):
  - `POST /api/v1/agency-requests` — customer requests agency (`budget_level` optional)
  - `POST /api/v1/admin/agency-requests/{assignment}/approve` — admin assigns agency (`role:admin|super_admin`)
  - `POST /api/v1/agency/assignments/{assignment}/approve` | `/decline` — agency responds (`role:agency`)
  - `POST /api/v1/agency/assignments/{assignment}/trips` — agency builds customer trip (items: `{type, id}`)
