(function (global) {
  ...
  function renderCategories(cats) {
    ...
    Array.prototype.forEach.call(listEl.querySelectorAll('.delete-btn'), function(btn) {
      btn.addEventListener('click', function() {
        ...
        It.apiDelete('/v1/categories/' + id, { auth: true })
          .then(function(res) {
            if (res.ok) {
  ...
  function fetchCategories() {
    It.apiGet('/v1/categories', { auth: true })
      .then(function(res) {
        var cats = res.data || res.body?.data || [];
    ...
    if (form) {
      form.addEventListener('submit', function(e) {
        if (form.method === 'dialog') {
            ...
            var req = id 
                ? It.apiPut('/v1/categories/' + id, payload, { auth: true })
                : It.apiPost('/v1/categories', payload, { auth: true });
