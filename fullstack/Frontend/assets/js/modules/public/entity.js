(function (global) {
  ...
  function imageHtml(src, alt, className) {
    ...
    return '<div class="relative overflow-hidden rounded-2xl border border-white/10 mb-6 bg-white/5">' +
  ...
  var TYPE_LABEL = { destination: "Destination", destinations: "Destination", hotel: "Hotel", hotels: "Hotel", restaurant: "Restaurant", restaurants: "Restaurant", attraction: "Attraction", attractions: "Attraction" };
  var ATTACH_TYPE = { hotel: "hotels", hotels: "hotels", restaurant: "restaurants", restaurants: "restaurants", attraction: "attractions", attractions: "attractions" };
  ...
  function metaHtml(e) {
    ...
    var d = e;
    var regionLabel = (d.region && d.region.label) || "";
    var countryName = (d.country && d.country.name) || "Destination";
    return '<div class="card__meta" style="display:flex;gap:0.5rem;flex-wrap:wrap;align-items:center;">' +
      '<span class="badge">' + It.app.esc(regionLabel ? regionLabel + " · " + countryName : countryName) + "</span>" +
      (d.hotels_count != null ? '<span class="badge badge--subtle">' + d.hotels_count + ' hotels</span>' : '') +
      (d.tours_count != null ? '<span class="badge badge--subtle">' + d.tours_count + ' tours</span>' : '') +
      (d.user_count ? '<span class="badge badge--subtle">' + d.user_count + ' travelers visited</span>' : '') +
  ...
  function render(e) {
    ...
    var bookSection = "";
    if (type === "destination") {
      bookSection = '<div class="card card--flat" style="border-left: 3px solid hsl(var(--primary));">' +
        '<p class="page-section__eyebrow">Instant Booking</p>' +
        '<h3 class="page-section__title" style="margin-bottom:var(--space-2);">Book a Tour Package</h3>' +
        '<p style="font-size:13px;color:hsl(var(--muted-foreground));margin-bottom:var(--space-3);">Secure your spot with verified local agency guides.</p>' +
        '<div style="display:flex;gap:0.5rem;margin-bottom:var(--space-3);">' +
        '<input type="number" id="book-guests" class="input input--sm" value="1" min="1" max="12" style="width:80px;" title="Guests" />' +
        '<button type="button" id="book-tour-btn" class="btn btn--primary btn--sm" style="flex:1;">Book Guided Tour</button></div></div>';
    }
    ...
    page.innerHTML =
      ...
      '</div><div id="entity-extra-section" style="margin-top:var(--space-4);"></div>' +
      '</div><aside style="display:flex;flex-direction:column;gap:var(--space-4);">' +
      bookSection +
    ...
    if (type === "destination" && el("book-tour-btn")) {
      el("book-tour-btn").addEventListener("click", function () {
        var user = It.session.getUser();
        if (!user) {
          It.app.showToast("Please log in to book a tour.", "error");
          setTimeout(function () { location.href = "/login.html"; }, 1500);
          return;
        }
        var guests = Math.max(1, Math.min(12, parseInt(el("book-guests").value, 10) || 1));
        var btn = this;
        btn.disabled = true;
        btn.textContent = "Booking...";

        It.apiPost("/v1/destinations/" + id + "/book", { guests: guests }, { auth: true }).then(function (res) {
          if (res.ok) {
            It.app.showToast("Tour booking initiated! Redirecting to checkout...", "success");
            setTimeout(function () { location.href = "/checkout.html"; }, 1200);
          } else {
            It.app.showToast((res.body && res.body.message) || "No tour packages are currently bookable for this destination.", "error");
            btn.disabled = false;
            btn.textContent = "Book Guided Tour";
          }
        }).catch(function () {
          It.app.showToast("Could not complete booking.", "error");
          btn.disabled = false;
          btn.textContent = "Book Guided Tour";
        });
      });
    ...
    if (type === "destination") {
      It.apiGet("/v1/destinations/" + id + "/hotels").then(function (res) {
        var hotels = It.app.unwrapData(res);
        if (!Array.isArray(hotels) || !hotels.length) return;
        var extra = el("entity-extra-section");
        if (!extra) return;
        var hotelsHtml = '<div class="card card--flat" style="margin-top:var(--space-4);">' +
          '<p class="page-section__eyebrow">Accommodations</p>' +
          '<h3 class="page-section__title" style="margin-bottom:var(--space-3);">Hotels in ' + It.app.esc(name) + '</h3>' +
          '<div style="display:grid;grid-template-columns:repeat(auto-fill, minmax(200px, 1fr));gap:0.75rem;">' +
          hotels.slice(0, 4).map(function (h) {
            return '<a href="/entity.html?type=hotel&id=' + h.id + '" class="card card--tile" style="border:1px solid hsl(var(--border));padding:0.75rem;">' +
              '<div style="font-weight:600;margin-bottom:0.25rem;">' + It.app.esc(h.name) + '</div>' +
              '<div style="font-size:12px;color:hsl(var(--muted-foreground));">' + (h.price_per_night ? '$' + h.price_per_night + '/night' : 'Available') + '</div></a>';
          }).join("") + '</div></div>';
        extra.innerHTML = hotelsHtml;
      }).catch(function () { /* ignore */ });
    ...
    if (type === "hotel") {
      It.apiGet("/v1/hotels/" + id + "/reviews").then(function (res) {
        var reviews = It.app.unwrapData(res);
        if (!Array.isArray(reviews) || !reviews.length) return;
        var extra = el("entity-extra-section");
        if (!extra) return;
        var reviewsHtml = '<div class="card card--flat" style="margin-top:var(--space-4);">' +
          '<p class="page-section__eyebrow">Verified Guest Feedback</p>' +
          '<h3 class="page-section__title" style="margin-bottom:var(--space-3);">Approved Reviews (' + reviews.length + ')</h3>' +
          '<div style="display:flex;flex-direction:column;gap:0.75rem;">' +
          reviews.map(function (r) {
            var userName = (r.user && r.user.name) || "Verified Guest";
            return '<div style="border-bottom:1px solid hsl(var(--border));padding-bottom:0.75rem;">' +
              '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.25rem;">' +
              '<strong style="font-size:13px;">' + It.app.esc(userName) + '</strong>' +
              '<span style="color:hsl(var(--warning, 38 92% 50%));font-size:12px;">★ ' + (r.rating || 5) + '</span></div>' +
              (r.comment ? '<p style="font-size:13px;color:hsl(var(--muted-foreground));margin:0;">' + It.app.esc(r.comment) + '</p>' : '') +
              '</div>';
          }).join("") + '</div></div>';
  ...
  function start() {
    if (!TYPE_LABEL[type] || !id) {
      page.innerHTML = '<div class="glass-card p-12 text-center max-w-lg mx-auto"><h2 class="text-2xl font-bold text-white mb-4">Item Not Found</h2>' +
        '<p class="text-white/60 mb-6">The requested catalog entity could not be located or does not exist.</p>' +
        '<a href="explore.html" class="btn-primary"><span>Back to Catalog Explorer</span><i class="fas fa-arrow-right text-xs"></i></a></div>';
      return;
    }

    var pluralType = (type === "destinations" || type === "hotels" || type === "restaurants" || type === "attractions") ? type : type + "s";
    It.apiGet("/" + pluralType + "/" + id).then(function (res) {
      if (res.ok && res.body) {
        ...
        page.innerHTML = '<div class="glass-card p-12 text-center max-w-lg mx-auto"><h2 class="text-2xl font-bold text-white mb-4">Could Not Load Item</h2>' +
          '<a href="explore.html" class="btn-primary"><span>Back to Catalog Explorer</span><i class="fas fa-arrow-right text-xs"></i></a></div>';
      }
    }).catch(function () {
