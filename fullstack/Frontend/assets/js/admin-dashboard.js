...
(function (global) {
  ...
  function load() {
    ...
    Promise.all([
      It.apiGet("/v1/admin/analytics?period=" + encodeURIComponent("30d"), { auth: true }),
      It.apiGet("/v1/admin/analytics/revenue?period=" + encodeURIComponent("30d"), { auth: true }),
      ...
      if (host) {
        ...
        const empty = document.createElement("div");
        empty.className = "kit-empty";
        empty.textContent = "Could not load bookings.";
        host.appendChild(empty);
      }
      It.feedback.banner("Could not load analytics.", "is-error");
    }).finally(function () {
      entrance();
    });
  }
