  ...
  function initPeriodSelector() {
    document.querySelectorAll(".chart-time-select").forEach(function (select) {
      select.addEventListener("change", function (e) {
        ...
        // If a global-period select exists, sync all and reload everything including KPIs
        // Otherwise just update the individual chart
        if (!chartType || chartType === "all") {
          globalPeriod = period;
          syncAllSelects(period);
          load(period);
        } else {
          // Check if there's a dedicated global selector; if not sync all & reload
          var globalSel = document.querySelector(".chart-time-select[data-chart='all']");
          if (!globalSel) {
            // No global selector — syncing all and reloading gives the best UX
            globalPeriod = period;
            syncAllSelects(period);
            load(period);
          } else {
