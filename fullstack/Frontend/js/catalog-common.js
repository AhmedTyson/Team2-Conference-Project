...
(function (global) {
  ...
  const ROUTES = {
    ...
    favsList: "/v1/dashboard/favourites",
  };
  ...
  /** Deterministic fallback image (seed-based) — same seed, same image. */
  function seedImg(item, w, h) {
    const seed = String(item && (item.id || item.name || "place")).toLowerCase().replace(/[^a-z0-9]+/g, "-");
    return "https://picsum.photos/seed/" + encodeURIComponent(seed) + "/" + (w || 640) + "/" + (h || 420);
  }

  /** Attach a live image to an <img> with seed fallback on failure. */
  function bindImg(img, item) {
    if (!img) return;
    const src = imgUrl(item && item.image);
    if (!src) {
      img.replaceWith(placeholderIcon());
      return;
    }
    img.onerror = function () {
      img.onerror = null;
      img.src = seedImg(item, img.dataset.w || 640, img.dataset.h || 420);
    };
    img.src = src;
  }
  ...
  function openReviewModal(type, id, name) {
    ...
    submitBtn.addEventListener("click", function () {
      ...
      It.apiPost("/v1/reviews/" + encodeURIComponent(type) + "/" + encodeURIComponent(id), payload, {})
        .then(function (res) {
