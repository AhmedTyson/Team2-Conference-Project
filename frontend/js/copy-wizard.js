...
nextBtn.addEventListener('click', async function () {
    if (currentStep === totalSteps - 1) {
        ...
        nextBtn.innerHTML = '<span class="spinner"></span> Copying…';
        try {
            var body = { title: title };
            if (start) body.start_date = start;
            if (end) body.end_date = end;
            await TP.api('/v1/trips/' + sourceTripId + '/copy', { method: 'POST', body: JSON.stringify(body) });
            TP.setFlash('Trip copied successfully!');
