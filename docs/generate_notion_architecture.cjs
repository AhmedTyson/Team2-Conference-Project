...
const PARENT_DB = 'database:884c2e77-8a84-46a8-852f-07651d2eed5e';
...
// 1. Create the Master Page Container first so we can nest subpages inside it
console.log('Creating Master Container Page...');
const masterFileInit = 'C:/Programming/conference/Team2-Conference-Project/docs/temp_master.md';
fs.writeFileSync(masterFileInit, '# 🌍 ThreeDOS Unified API Specification\nGenerating modules...');
const masterPageOutput = execSync(`npx --yes ntn pages create --parent ${PARENT_DB} < "${masterFileInit}"`).toString().trim();
const masterPageId = masterPageOutput;
console.log('Master Page ID:', masterPageId);

let subpageLinks = [];
let fullTablesMd = '';
let globalCounter = 1;

// 2. Create subpages sequentially and capture their IDs
for (let mod of modules) {
  console.log(`Creating Subpage: ${mod.title}...`);
  let subpageMd = `# ${mod.title}\n\n<callout icon="ℹ️" color="blue_bg">\n\tDetailed API contracts for ${mod.title}. Request/Response payloads will be defined here by the assigned developer.\n</callout>\n\n## Endpoint Summary\n`;
  subpageMd += renderTable(mod.endpoints, mod.assignee);
  subpageMd += '\n## Detailed Specifications\n<callout icon="🚧" color="yellow_bg">\n\tPayload schemas to be added.\n</callout>';
  
  const subFile = `C:/Programming/conference/Team2-Conference-Project/docs/temp_${globalCounter}.md`;
  fs.writeFileSync(subFile, subpageMd);
  
  // Note: We place the subpages inside the Master Page!
  const subOut = execSync(`npx --yes ntn pages create --parent page:${masterPageId} < "${subFile}"`).toString().trim();
  subpageLinks.push(`<page url="https://app.notion.com/p/${subOut.replace(/-/g, '')}">${mod.title}</page>`);
  
  // Append to the giant table section for the master page
  fullTablesMd += `### 1.${globalCounter} ${mod.title.replace(/[\u{1F300}-\u{1F9FF}]/gu, '').trim()}\n`;
  fullTablesMd += renderTable(mod.endpoints, mod.assignee) + '\n';
...
// 3. Construct Final Master Page Markdown
...
fs.writeFileSync('C:/Programming/conference/Team2-Conference-Project/docs/ThreeDOS_Notion_Final_Master.md', finalMasterMd);

// 4. Update the Master Page
console.log('Overwriting Master Page with full structure...');
execSync(`npx --yes ntn pages edit ${masterPageId} < "C:/Programming/conference/Team2-Conference-Project/docs/ThreeDOS_Notion_Final_Master.md"`);
console.log('Complete! Master Page ID: ' + masterPageId);
