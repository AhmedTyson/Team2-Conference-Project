---
Author: ''
Concept Tags: []
Date Saved: 2026-08-02T01:01:00.000Z
My Notes: ''
Post Type: null
Relevance: null
Source Type: null
Status: null
Summary: ''
Tech Tags: []
Title: 🌍 ThreeDOS Unified API Specification
Topic Tags: []
---

# 🌍 ThreeDOS Unified API Specification
<callout icon="💡" color="blue_bg">
	**Purpose of this document:** <span color="yellow_bg">frontend/backend integration contract. This is the source used to generate OpenAPI/Postman collections. Every endpoint below defines exactly what the frontend sends and what it gets back — no backend implementation detail included, since that's not where integration problems come from.</span>
</callout>
---
## 🗂️ Table of Contents (Modules)
### 🏃‍♂️ Sprint 1 (Active)
<page url="https://app.notion.com/p/3b02c9c07e6681ad86a7c8d57086c462">🔐 Auth Module</page>
<page url="https://app.notion.com/p/3b02c9c07e668183969cf53adfdfc5a4">📋 User Onboarding (Survey)</page>
<page url="https://app.notion.com/p/3b02c9c07e668108afa4f97a3482b704">🗺️ Trip Planner Engine</page>
<page url="https://app.notion.com/p/3b02c9c07e668180853ff3dd092c284b">🌍 Explore Directory</page>
<page url="https://app.notion.com/p/3b02c9c07e6681ca9fc6f5c1f27ad679">🏷️ Categories Module</page>
<page url="https://app.notion.com/p/3b02c9c07e66819e8450c1175e23de0b">💬 User Interactions (Community)</page>
### 📅 Future Sprints (Backlog)
<page url="https://app.notion.com/p/3b02c9c07e6681c6a062f4ffe8b56f3a">🤖 AI & API Proxies</page>
<page url="https://app.notion.com/p/3b02c9c07e6681fdb100c765961df9fc">📍 Interactive Maps</page>
<page url="https://app.notion.com/p/3b02c9c07e6681b9b838da389c11327e">📊 User Dashboard</page>
<page url="https://app.notion.com/p/3b02c9c07e6681589f33f7c721010856">💳 Payments & Transactions (Pending)</page>
<page url="https://app.notion.com/p/3b02c9c07e6681149145c822118ef3c4">👥 Admin Users</page>
<page url="https://app.notion.com/p/3b02c9c07e6681aab1d8fb635992e0e5">✈️ Admin Trips</page>
<page url="https://app.notion.com/p/3b02c9c07e6681f3a700e95e4ae51090">📍 Admin Destinations</page>
<page url="https://app.notion.com/p/3b02c9c07e6681e5b9ebe0917d5e1d71">🏨 Admin Hotels & Restaurants</page>
<page url="https://app.notion.com/p/3b02c9c07e66811786c7c3399dc05513">🏷️ Admin Categories</page>
<page url="https://app.notion.com/p/3b02c9c07e668101891feb764bed6bda">🚨 Admin Reviews Moderation</page>
<page url="https://app.notion.com/p/3b02c9c07e6681349088f1884a9ddf49">✉️ Admin Contacts Inbox</page>
<page url="https://app.notion.com/p/3b02c9c07e6681e7abb0c834b708800b">⚙️ Admin Analytics & Settings</page>
<page url="https://app.notion.com/p/3b02c9c07e66814b825fe9068b43daac">📄 Admin Reports (Pending)</page>
---
## Global Conventions

### 🏗️ API Architecture & Versioning
- **Base URL:** All endpoints are prefixed with `/api/v1/v1/` to ensure future backward compatibility.
- **Pagination:** All collection endpoints return a `meta` object containing pagination details.

### ☁️ File Storage (Railway Constraints)
<callout icon="☁️" color="blue_bg">
	**S3 Absolute URLs:** Railway uses ephemeral storage. All image uploads (Profiles, Trips) are saved to S3. The API will always return **absolute HTTPS URLs** for `image` fields.
</callout>

### ⏳ Asynchronous Processing & Polling
<callout icon="⏱️" color="orange_bg">
	**202 Accepted:** Endpoints hitting OpenAI (e.g., `/generate-ai`) execute via background jobs to prevent HTTP timeouts. The API returns `202 Accepted` immediately. Frontend must poll the resource or listen for websocket events.
</callout>

### 🛑 Strict Rate Limiting
- **Global API Limit:** 60 requests per minute per IP/User.
- **Paid Endpoints Limit:** Routes utilizing OpenAI or RapidAPI are heavily throttled to **3 requests per minute**. Exceeding this yields a `429 Too Many Requests`.

<callout icon="🚨" color="red_bg">
	**FRONTEND TEAM:** The access token MUST be sent in the `Authorization: Bearer {token}` HTTP header. **NEVER** put the token inside the JSON request body.
</callout>
### Standard Response Envelope
<callout icon="✅" color="green_bg">
	**Success:**
	```json
{
  "success": true,
  "message": "Human-readable message",
  "data": { }
}
	```
</callout>
<callout icon="🚨" color="red_bg">
	**Error:**
	```json
{
  "success": false,
  "message": "Human-readable error message",
  "errors": { }
}
	```
	<span color="red">Note:</span> `errors` is only present on `422` validation failures.
</callout>
### Status Code Legend (used across every endpoint)
<table header-row="true">
<tr>
<td>Code</td>
<td>Meaning</td>
<td>When</td>
</tr>
<tr>
<td>`200`</td>
<td>OK</td>
<td>Successful GET/PUT/POST that doesn't create a resource</td>
</tr>
<tr>
<td>`201`</td>
<td>Created</td>
<td>Successful POST that creates a new resource</td>
</tr>
<tr>
<td>`204`</td>
<td>No Content</td>
<td>Successful DELETE</td>
</tr>
<tr>
<td>`400`</td>
<td>Bad Request</td>
<td>Malformed request (rare → usually caught by 422 instead)</td>
</tr>
<tr>
<td>`401`</td>
<td>Unauthorized</td>
<td>Missing/invalid/expired token</td>
</tr>
<tr>
<td>`403`</td>
<td>Forbidden</td>
<td>Valid token, but wrong role or not the resource owner</td>
</tr>
<tr>
<td>`404`</td>
<td>Not Found</td>
<td>Resource ID doesn't exist</td>
</tr>
<tr>
<td>`422`</td>
<td>Unprocessable Entity</td>
<td>Validation failed</td>
</tr>
<tr>
<td>`429`</td>
<td>Too Many Requests</td>
<td>Rate limit hit</td>
</tr>
<tr>
<td>`500`</td>
<td>Server Error</td>
<td>Unhandled failure — frontend should show generic error state</td>
</tr>
</table>
### Access Level Key
- 🌐 **Public** → no token needed
- 🔒 **Auth** → any logged-in user (customer or admin)
- 🛡️ **Admin** → logged-in user with `role = admin`
- 👤 **Owner** → logged-in user, and must own the resource (checked server-side via Policy)
---
## 1. Quick Reference — Endpoint Summary Tables
### 🏃‍♂️ Sprint 1 (Deadline: Sunday 11:59 pm)
> Backend tasks assigned based on team structure.
### 1.1 Auth Module
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>POST</td>
<td>`/api/v1/auth/register`</td>
<td>User Registration.</td>
<td>🌐 Public</td>
<td>SARA / LOJY</td>
</tr>
<tr>
<td>2</td>
<td>POST</td>
<td>`/api/v1/auth/login`</td>
<td>Login / Role-Based Access Setup.</td>
<td>🌐 Public</td>
<td>SARA / LOJY</td>
</tr>
<tr>
<td>3</td>
<td>POST</td>
<td>`/api/v1/auth/logout`</td>
<td>Invalidate session token.</td>
<td>🔒 Auth</td>
<td>SARA / LOJY</td>
</tr>
<tr>
<td>4</td>
<td>POST</td>
<td>`/api/v1/auth/forgot-password`</td>
<td>Request password reset.</td>
<td>🌐 Public</td>
<td>SARA / LOJY</td>
</tr>
<tr>
<td>5</td>
<td>POST</td>
<td>`/api/v1/auth/verify-email`</td>
<td>Verify Email.</td>
<td>🔒 Auth</td>
<td>SARA / LOJY</td>
</tr>
<tr>
<td>6</td>
<td>GET</td>
<td>`/api/v1/profile`</td>
<td>Fetch Profile Management data.</td>
<td>👤 Owner</td>
<td>SARA / LOJY</td>
</tr>
<tr>
<td>7</td>
<td>PATCH</td>
<td>`/api/v1/profile`</td>
<td>Update Profile Management data.</td>
<td>👤 Owner</td>
<td>SARA / LOJY</td>
</tr>
</table>
### 1.2 User Onboarding (Survey)
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET</td>
<td>`/api/v1/onboarding`</td>
<td>Get user preferences.</td>
<td>👤 Owner</td>
<td>SAMA</td>
</tr>
<tr>
<td>2</td>
<td>POST</td>
<td>`/api/v1/onboarding`</td>
<td>Save Travel Style, Budget, Interests.</td>
<td>👤 Owner</td>
<td>SAMA</td>
</tr>
</table>
### 1.3 ️ Trip Planner Engine
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET</td>
<td>`/api/v1/trips/create`</td>
<td>Select Destination, Days, Budget.</td>
<td>🔒 Auth</td>
<td>FADY / ADHAM</td>
</tr>
<tr>
<td>2</td>
<td>POST</td>
<td>`/api/v1/trips`</td>
<td>Save basic Trip parameters.</td>
<td>🔒 Auth</td>
<td>FADY / ADHAM</td>
</tr>
<tr>
<td>3</td>
<td>GET</td>
<td>`/api/v1/trips/{trip}`</td>
<td>View Daily Travel Itinerary.</td>
<td>👤 Owner</td>
<td>FADY / ADHAM</td>
</tr>
<tr>
<td>4</td>
<td>POST</td>
<td>`/api/v1/trips/{trip}/attach/{type}`</td>
<td>Attach Hotels / Restaurants.</td>
<td>👤 Owner</td>
<td>FADY / ADHAM</td>
</tr>
<tr>
<td>5</td>
<td>DELETE</td>
<td>`/api/v1/trips/{trip}/detach/{id}`</td>
<td>Remove attached items.</td>
<td>👤 Owner</td>
<td>FADY / ADHAM</td>
</tr>
</table>
### 1.4 Explore Directory
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET</td>
<td>`/api/v1/destinations`</td>
<td>List all destinations + filters.</td>
<td>🌐 Public</td>
<td>KENZY / HANA</td>
</tr>
<tr>
<td>2</td>
<td>GET</td>
<td>`/api/v1/destinations/{id}`</td>
<td>Destination details + Leaflet map.</td>
<td>🌐 Public</td>
<td>KENZY / HANA</td>
</tr>
<tr>
<td>3</td>
<td>GET</td>
<td>`/api/v1/hotels`</td>
<td>List hotels + search & filter.</td>
<td>🌐 Public</td>
<td>KENZY / HANA</td>
</tr>
<tr>
<td>4</td>
<td>GET</td>
<td>`/api/v1/hotels/{id}`</td>
<td>Hotel details.</td>
<td>🌐 Public</td>
<td>KENZY / HANA</td>
</tr>
<tr>
<td>5</td>
<td>GET</td>
<td>`/api/v1/restaurants`</td>
<td>List restaurants + filter.</td>
<td>🌐 Public</td>
<td>KENZY / HANA</td>
</tr>
<tr>
<td>6</td>
<td>GET</td>
<td>`/api/v1/restaurants/{id}`</td>
<td>Restaurant details.</td>
<td>🌐 Public</td>
<td>KENZY / HANA</td>
</tr>
<tr>
<td>7</td>
<td>GET</td>
<td>`/api/v1/attractions`</td>
<td>List attractions.</td>
<td>🌐 Public</td>
<td>KENZY / HANA</td>
</tr>
<tr>
<td>8</td>
<td>GET</td>
<td>`/api/v1/attractions/{id}`</td>
<td>Attraction details.</td>
<td>🌐 Public</td>
<td>KENZY / HANA</td>
</tr>
</table>
### 1.5 ️ Categories Module
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET</td>
<td>`/api/v1/categories`</td>
<td>List categories as clickable cards.</td>
<td>🌐 Public</td>
<td>RANA</td>
</tr>
<tr>
<td>2</td>
<td>GET</td>
<td>`/api/v1/categories/{id}`</td>
<td>View everything in a specific category.</td>
<td>🌐 Public</td>
<td>RANA</td>
</tr>
</table>
### 1.15 ️ Admin Categories
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET/POST/PUT</td>
<td>`/api/v1/admin/categories`</td>
<td>Manage Beaches, Mountains, etc.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
</table>
### <br>1.6 User Interactions (Community)
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>POST</td>
<td>`/api/v1/favourites/{type}/{id}`</td>
<td>Add/Remove Favorite (Polymorphic).</td>
<td>🔒 Auth</td>
<td>TYSON</td>
</tr>
<tr>
<td>2</td>
<td>POST</td>
<td>`/api/v1/reviews/{type}/{id}`</td>
<td>Submit a review (saved as pending).</td>
<td>🔒 Auth</td>
<td>TYSON</td>
</tr>
<tr>
<td>3</td>
<td>DELETE</td>
<td>`/api/v1/reviews/{id}`</td>
<td>Delete user review.</td>
<td>👤 Owner</td>
<td>TYSON</td>
</tr>
</table>
---
### 📅 Future Sprints (Backlog)
> These modules are scheduled for later sprints.
### 1.7 AI & API Proxies
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET</td>
<td>`/api/v1/weather/{city}`</td>
<td>Weather API: Current weather, temp, wind.</td>
<td>🌐 Public</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>2</td>
<td>POST</td>
<td>`/api/v1/trips/{trip}/generate-ai`</td>
<td>Trigger OpenAI Itinerary Generation.</td>
<td>👤 Owner</td>
<td>FUTURE SPRINTS</td>
</tr>
</table>
### 1.8 Interactive Maps
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET</td>
<td>`/api/v1/maps/destination/{id}`</td>
<td>Attractions, Hotels, Restaurants Locations.</td>
<td>🌐 Public</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>2</td>
<td>GET</td>
<td>`/api/v1/maps/trip/{id}`</td>
<td>Route Directions between trip itinerary points.</td>
<td>👤 Owner</td>
<td>FUTURE SPRINTS</td>
</tr>
</table>
### 1.9 User Dashboard
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET</td>
<td>`/api/v1/dashboard`</td>
<td>Trip Statistics & Overview.</td>
<td>👤 Owner</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>2</td>
<td>GET</td>
<td>`/api/v1/dashboard/trips`</td>
<td>Saved Trips & Booking History.</td>
<td>👤 Owner</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>3</td>
<td>GET</td>
<td>`/api/v1/dashboard/favourites`</td>
<td>Favorite Destinations & Places.</td>
<td>👤 Owner</td>
<td>FUTURE SPRINTS</td>
</tr>
</table>
### 1.10 Payments & Transactions (Pending)
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>POST</td>
<td>`/api/v1/payments/checkout`</td>
<td>Init Stripe/PayPal booking session.</td>
<td>🔒 Auth</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>2</td>
<td>POST</td>
<td>`/api/v1/payments/webhook`</td>
<td>Receive payment gateway callbacks.</td>
<td>🌐 Public</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>3</td>
<td>GET</td>
<td>`/api/v1/transactions`</td>
<td>List user payment history/receipts.</td>
<td>👤 Owner</td>
<td>FUTURE SPRINTS</td>
</tr>
</table>
### 1.11 Admin Users
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET</td>
<td>`/api/v1/admin/users`</td>
<td>View Users.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>2</td>
<td>POST</td>
<td>`/api/v1/admin/users`</td>
<td>Add Users.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>3</td>
<td>PUT</td>
<td>`/api/v1/admin/users/{id}`</td>
<td>Edit Users.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>4</td>
<td>PATCH</td>
<td>`/api/v1/admin/users/{id}/activate`</td>
<td>Activate Account.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>5</td>
<td>PATCH</td>
<td>`/api/v1/admin/users/{id}/block`</td>
<td>Block Account.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
</table>
### 1.12 ✈️ Admin Trips
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET</td>
<td>`/api/v1/admin/trips`</td>
<td>View Trips.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>2</td>
<td>PUT</td>
<td>`/api/v1/admin/trips/{id}`</td>
<td>Edit Trips.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>3</td>
<td>DELETE</td>
<td>`/api/v1/admin/trips/{id}`</td>
<td>Delete Trips.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
</table>
### 1.13 Admin Destinations
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET/POST/PUT</td>
<td>`/api/v1/admin/countries`</td>
<td>Countries CRUD.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>2</td>
<td>GET/POST/PUT</td>
<td>`/api/v1/admin/destinations`</td>
<td>Destinations/Cities CRUD.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>3</td>
<td>GET/POST/PUT</td>
<td>`/api/v1/admin/attractions`</td>
<td>Attractions CRUD.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
</table>
### 1.14 Admin Hotels & Restaurants
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET/POST/PUT</td>
<td>`/api/v1/admin/hotels`</td>
<td>Hotels CRUD.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>2</td>
<td>GET/POST/PUT</td>
<td>`/api/v1/admin/restaurants`</td>
<td>Restaurants CRUD.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
</table>
### 1.15 ️ Admin Categories
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET/POST/PUT</td>
<td>`/api/v1/admin/categories`</td>
<td>Manage Beaches, Mountains, etc.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
</table>
### 1.16 Admin Reviews Moderation
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET</td>
<td>`/api/v1/admin/reviews`</td>
<td>View all reviews.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>2</td>
<td>PATCH</td>
<td>`/api/v1/admin/reviews/{id}/approve`</td>
<td>Approve Review (Publish).</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>3</td>
<td>PATCH</td>
<td>`/api/v1/admin/reviews/{id}/reject`</td>
<td>Reject Review (Hide).</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>4</td>
<td>DELETE</td>
<td>`/api/v1/admin/reviews/{id}`</td>
<td>Delete Reviews.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
</table>
### 1.17 ✉️ Admin Contacts Inbox
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET</td>
<td>`/api/v1/admin/contacts`</td>
<td>Manage user inquiries.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>2</td>
<td>PATCH</td>
<td>`/api/v1/admin/contacts/{id}/read`</td>
<td>Mark message as Read.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>3</td>
<td>PATCH</td>
<td>`/api/v1/admin/contacts/{id}/resolve`</td>
<td>Mark message as Resolved.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
</table>
### 1.18 ⚙️ Admin Analytics & Settings
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET</td>
<td>`/api/v1/admin/analytics`</td>
<td>Analytics Charts (Users, Revenue).</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>2</td>
<td>GET</td>
<td>`/api/v1/admin/settings`</td>
<td>Manage Logo, Site Name, Socials.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>3</td>
<td>PUT</td>
<td>`/api/v1/admin/settings`</td>
<td>Save Settings.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
</table>
### 1.19 Admin Reports (Pending)
<table header-row="true">
<tr>
<td>#</td>
<td>Method</td>
<td>Route</td>
<td>Description</td>
<td>Authorize</td>
<td>Backend</td>
</tr>
<tr>
<td>1</td>
<td>GET</td>
<td>`/api/v1/admin/reports`</td>
<td>List generated PDF/CSV exports.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
<tr>
<td>2</td>
<td>POST</td>
<td>`/api/v1/admin/reports/generate`</td>
<td>Generate Revenue/Growth reports.</td>
<td>🛡️ Admin</td>
<td>FUTURE SPRINTS</td>
</tr>
</table>